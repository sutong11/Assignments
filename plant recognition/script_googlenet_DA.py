...
def train():
    ...
    #model = build_vgg19(img_rows, img_cols, num_classes)
    
    # create the base pre-trained model
    #base_model = InceptionV3(weights='imagenet', include_top=False, input_shape=(img_rows, img_cols, img_channels))
    
    # add layers
    #m = base_model.output
    #m = GlobalAveragePooling2D()(m)
    #m = Dense(1024, name='fc1')(m)
    #m = BatchNormalization(axis=-1, name='fc1_bn')(m)
    #m = Activation('relu', name='fc1_func')(m)
    #m = Dropout(0.5)(m)
    #predictions = Dense(num_classes, activation='softmax', name='predictions')(m)
    
    #model = Model(inputs=base_model.input, outputs=predictions)
    model = load_model('./model_results/incep_checkpoint.hdf5')
    
    #for layer in base_model.layers:
    #    layer.trainable = False

    # calculate class weight
    labels_array = np.array(id_newlabel_train.values())
    class_weights = sklearn.utils.class_weight.compute_class_weight('balanced', np.unique(labels_array), labels_array)
    
    earlystop = EarlyStopping(monitor='val_loss',min_delta=0.00001, patience=5, verbose=1, mode='auto' )
    checkpointer = ModelCheckpoint(filepath='./model_results/checkpoint.hdf5', monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=False, mode='auto', period=1 )
    ...
    if not data_augmentation:
        ...
        model_info = model.fit_generator(generator(batch_size), steps_per_epoch=(total_num_train//batch_size+1), epochs=epochs,verbose=1, callbacks=callbacks, class_weight=class_weights, validation_data=(x_validation, y_validation))
        ...
        model_info = model.fit_generator(augmentation_generator(batch_size, datagen),
                            steps_per_epoch=(total_num_train//batch_size+1)*3,
                            epochs=epochs,
                            verbose=1,
                            callbacks=callbacks,
    ...
    model.save('./model_results/inceptionv3_DA_bottleneck.h5')
    model_history = model_info.history
    cPickle.dump(model_history, open('./History/inceptionv3_DA_bottleneck_history.pkl', 'wb'))
    '''
    ...
    # the first 12 layers and unfreeze the rest:
    ...
    model.save('./model_results/inceptionv3_pretrained_DA.h5')
    model_history = model_info.history
    cPickle.dump(model_history, open('./History/inceptionv3_pretrained_DA_history.pkl', 'wb'))
...
if __name__ == '__main__':
    ...
    if loadmodel:
        model = load_model('inceptionv3_pretrained_DA.h5')
