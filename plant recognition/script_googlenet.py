...
def train():
    ...
    if not data_augmentation:
        ...
        model_info = model.fit_generator(generator(batch_size), steps_per_epoch=(total_num_train//batch_size+1), epochs=epochs,verbose=1, callbacks=callbacks, class_weight=class_weights, max_q_size=1, validation_data=(x_validation, y_validation))
    ...
    model.save('./model_results/vgg19_pretrained_DA.h5')
    model_history = model_info.history
    cPickle.dump(model_history, open('./History/vgg19_pretrained_DA_history.pkl', 'wb'))
...
if __name__ == '__main__':
    ...
    if loadmodel:
        model = load_model('vgg19_pretrained_DA.h5')
