...
def vgg19_model():
    ...
    m = Dense(4096, name='fc1')(m)
    m = BatchNormalization(axis=-1, name='fc1_bn')(m)
    m = Activation('relu', name='fc1_func')(m)
    m = Dropout(0.5)(m)
    
    m = Dense(4096, name='fc2')(m)
    m = BatchNormalization(axis=-1, name='fc2_bn')(m)
    m = Activation('relu', name='fc2_func')(m)
    m = Dropout(0.5)(m)
...
def train():
    ...
    earlystop = EarlyStopping(monitor='val_loss',min_delta=0.00001, patience=3, verbose=1, mode='auto' )
    callbacks = [earlystop]
    ...
    if not data_augmentation:
        ...
        model_info = model.fit_generator(generator(batch_size), steps_per_epoch=(total_num_train//batch_size+1), epochs=epochs,verbose=1, callbacks=callbacks, max_q_size=1, class_weight=class_weights, validation_data=(x_validation, y_validation))
    ...
    model.save('./model_results/vgg19_pretrained_DA_real.h5')
    model_history = model_info.history
    cPickle.dump(model_history, open('./History/vgg19_pretrained_DA_real_history.pkl', 'wb'))
...
if __name__ == '__main__':
    ...
    if loadmodel:
        model = load_model('vgg19_pretrained_DA_real.h5')
